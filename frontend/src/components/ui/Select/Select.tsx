...
import { FunctionComponent, MouseEventHandler, ReactNode, useEffect, useRef, useState } from "react";
...
export const Select: FunctionComponent<SelectProps> = ({ placeholder, options = [], value, setValue }): ReactNode => {
    const [isOpen, setIsOpen] = useState<boolean>(false);
    const selectRef = useRef<HTMLSelectElement | null>(null);
    const dropdownRef = useRef<HTMLDivElement | null>(null);
    const selectStyles = classNames("border-radius-md", sfPro.className, styles.select, value === "placeholder" ? styles.placeholder : undefined);
    ...
    return (
        <div className={styles.wrapper}>
            <select
                ...
                ref={selectRef}
            >
                {Boolean(placeholder) && <option disabled value="placeholder">{placeholder}</option>}
                {options.map(({ text, value }): ReactNode => <option key={value} value={value}>{text}</option>)}
            </select>
            <Icon.Arrow
